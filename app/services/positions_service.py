...
class PositionsService:
    ...
    @staticmethod
    def invalidate_open_positions_cache() -> None:
        """写路径在 open_positions 变更后调用，触发下次读懒加载。"""
    ...
    async def build_open_positions_response(self, db, client=None, since_version: int | None = None):
        ...
        payload = await self._read_model.get_or_load_async(
            cache_key,
            lambda: self._build_open_positions_payload(db, today_snapshot_date, now),
    ...
    async def _build_open_positions_payload(self, db, today_snapshot_date: str, now: datetime) -> dict:
