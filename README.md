...
- **资产实时监控**:
  ...
  - **核心算法**: 自动计算**累计净值 (Cumulative Equity)**，剔除充提币影响，准确反映策略真实表现。
  - **风险监控**: 实时展示当前回撤、历史最大回撤 (Period Max DD)、波动率和账户健康分。
  - **绩效指标**: 集成机构级评价指标，包括 **夏普比率 (Sharpe Ratio)**、**卡玛比率 (Calmar Ratio)** 和 **时间胜率 (Win Rate)**。
- **后台任务**:
